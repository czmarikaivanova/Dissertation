...
Different sources in $G$ form MBT instances with generally different minimum broadcast times.
...
The value of $\tau(G,S)$ can be retrievd from the maximumg among the labels assigned to nodes.
...
takes longer time copared to the model presented in Paper IV in virtually all tested instances (see Tab.~\ref{tab:xx}).
However, there is a potential in improving the model such as further symmetry removal and additional valid inequalities following from the concept of unique numbering of binomial trees.
