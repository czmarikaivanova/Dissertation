...
A WANET is modeled by a complete weighted graph $(G=(V,E),p)$, where the function $p:E\to \mathbb{R}^+$ indicates the power requirement for sending a signal along a certain edge.
This requirement depends on distance and environmental properties. 
Assuming a homogenous environment, $p_{ij}=d_{ij}^\alpha$, 
where $d_{ij}$ is a distance between the devices represented by vertices $i$ and $j$, and $\alpha$ is a constant typically valued between 2 and 4. 
...
The disadvantages by which the MEB concept is facing is addressed by the range assignemtn problem (RAP).
\begin{problem}
Find a power vector $(P_1,P_2,\dots,P_n)\in\mathbb{R}^n$ of minimum sum s. t. the induced graph $G'=(V,E^P)$ is connected, where $E^P=\{(i,j)\in E: p_{ij}\leq P_i\wedge p_{ij}\leq P_j\}$.
\end{problem}
