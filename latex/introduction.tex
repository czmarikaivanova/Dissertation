...
that adequate power is provided to the nodes to ensure that the induced transmission graph is strongly connected.
For instances consisting of colinear points, there exists an algorithm that solves the problem to optimality in $\mathcal{O}(n^4)$ \cite{kirousis00}.
In \cite{clementi99} it is shown that RAP is NP-hard for instances in a $d$-dimensional Euclidean space for $d\geq 2$, and that for $d=3$, the problem is APX-complete.
...
RAP has been studied in literature
