...
An instance of an optimization problem cosists of a set of feasible solutions together with a cost function and a target which can be either minimization or maximization. 
\begin{definition}
	Given an algorithm $\mathcal{A}$ and a  minimization problem $A$ with a cost function $c$ and its instance $I$.
	We say that $\mathcal{A}$ is a $\rho$-approximation algorithm for $A$ if 
	$$
	\max\left\{\frac{c(\mathcal{A}(I))}{opt(I)}\right\}\leq\rho,
	$$
	where $opt(I)$ is the cost of an optimal solution to $I$.
\end{definition}
